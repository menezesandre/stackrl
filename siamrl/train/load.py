...
def load(
  ...
  try:
    ...
    if config_file is not None:
      ...
      net = nets.DeepQSiamFCN
      for key in gin.config._CONFIG:
        if key[-1].endswith('Training'):
          if 'net' in gin.config._CONFIG[key]:
            net = gin.config._CONFIG[key]['net'].configurable.fn_or_cls
          break
      with gin.config_scope('load'):
        net = net(_observation_spec)
      # Restore config state
      gin.config._CONFIG = _config
    else:
      net = nets.DeepQSiamFCN(_observation_spec)
...
def load_model(
  ...
  **kwargs,
  ...
  """Load a model saved from training.
    ...
    if config_file is not None:
      ...
      for key in gin.config._CONFIG:
        if key[-1].endswith('Training'):
          ...
          break
      with gin.config_scope('load'):
        net = net(observation_spec, **kwargs)
      # Restore config state
      gin.config._CONFIG = _config
    else:
      net = nets.DeepQSiamFCN(observation_spec, **kwargs)

    net.load_weights(os.path.join(wpath,'weights'))
    if verbose:
      print('Weights loaded from {}'.format(wpath), **kwargs)
