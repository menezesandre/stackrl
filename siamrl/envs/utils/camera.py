...
class Camera(object):
  def __init__(self, targetPos = [0,0,0], cameraPos = [0,0,10**3], upVector = 
  ...
  def getImage(self, physicsClient = None):
    return p.getCameraImage(self.width, self.height, viewMatrix = self.viewMatrix, 
      projectionMatrix = self.projectionMatrix, physicsClientId=physicsClient)
...
class ElevationCamera(Camera):
  ...
  def __call__(self, physicsClient = None, flip = None):
    _,_,_,d,_ = self.getImage(physicsClient)
    d = (1.-d)*self.range
    if flip is not None:
